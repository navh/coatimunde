...
%I fully intend to talk a lot more here about all of the
...
The change of schedule document was handed in at the beginning of January \cite{schupd}. This outlined any deadlines that were modified due to delays in other functionalities. The area that look longer than expected was getting the camera to consistently recognize ArUco markers on the Turtlebot. This extension caused all other deadline to get delayed or shorten as this task was on the critical path for the project. 
...
\section{Design}
	...
	Given the limited field of view of the robot, it cannot always be able to see the target. The robot, in the pfinder node, saves a dictionary of points that are associated with targets that have been resolved. Only the most recent 'citing' of a target is recorded, but as the robot moves its position relative to that target is updated as well. %Again I think that we need to think seriously about how we are going to try to use tha t mapping software stuff, fuck me up. 
